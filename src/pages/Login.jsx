    ...
    <div className=' w-1/2 bg-lightest'>
       ...
       <img src={pettern6} className='animate__animated animate__fadeIn animate__delay-800ms w-24 bottom-0 right-0 absolute' alt="" />
        <div className='w-1/4 h-2/3  absolute  top-1/2 transform translate-x-1/2 -translate-y-1/2'>
            <h1 className='font-body font-bold text-2xl text-secound'>Masuk Akun</h1>
            <p className='font-body text-base text-dark1'>Buat kamu yang sudah daftar, silahkan masuk akunmu.</p>
            <form action="">
                ...
                <input className='w-full bg-transparent  mb-3 focus:border-secound mt-5 outline-none border-b font-body border-primary' type="password" placeholder="Password" />
                <Link to='/Home' className='font-body hover:text-primary2 text-dark1 float-right'>Lupa Password?</Link>
                <button className='mt-6 px-36 font-bold py-2 rounded-md hover:bg-secound hover:text-white text-dark1 shadow-md bg-white font-body text-primary'>Masuk</button>
            </form>
            ...
            <p className='font-body mt-5 text-dark1'>Belum punya akun?  <Link to='/Register' className='hover:text-primary2'>Daftar Sekarang</Link></p>
        </div>
    </div>
